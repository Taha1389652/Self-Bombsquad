...
class QuakeGame(bs.TeamGameActivity[Player, Team]):
    ...
    creator = "Telegram : @BsRushGames"
    description = f"Kill a set number of enemies to win.\n\n{creator}\nMods : @BsRush_Mod\nGap : @BsRush_Gap"
    ...
    def on_begin(self) -> None:
        ...
        self._add_iran_text()
        self._setup_time_display()  # اضافه کردن نمایش زمان
    ...
    def _add_iran_text(self) -> None:
        self.iran_text = bs.newnode('text',
                         attrs={
                             'text': f'{self.creator}\nMods : @BsRush_Mod\nGap : @BsRush_Gap',
                             'scale': 0.8,
                             'position': (0, 30),
        ...
        self.quake_text = bs.newnode('text',
                         attrs={
                             'text': '⚡BsRush⚡',
                             'scale': 3.0,        # کمی بزرگتر
                             'position': (0, 600), 
                             'shadow': 0.5,
                             'flatness': 1.0,
                             'color': (1, 1, 1),
        ...
        rainbow_animate = {
            ...
            8: (0, 0, 1),      # آبی
            10: (1, 0, 1),     # بنفش
            12: (1, 0, 0)      # بازگشت به قرمز
        }
        
        bs.animate_array(cmb, 'output', 3, rainbow_animate, loop=True)
        cmb.connectattr('output', self.iran_text, 'color')

    def _setup_time_display(self) -> None:
        """Setup the time display text"""
        # ایجاد متن برای نمایش زمان
        self._time_text = bs.newnode('text',
            attrs={
                'text': '00:00',
                'scale': 1.2,
                'position': (650, 0),  # موقعیت در سمت راست
                'shadow': 0.5,
                'flatness': 1.0,
                'color': (1, 1, 1),
                'h_align': 'right',
        ...
        # تایمر برای به‌روزرسانی زمان هر ثانیه
        self._time_update_timer = bs.Timer(1.0, bs.WeakCall(self._update_time_display), repeat=True)
    ...
    def _update_time_display(self) -> None:
        ...
        elapsed_time = bs.time() - self._start_time
    ...
    def end_game(self) -> None:
        """پاک کردن تایمر و متن هنگام پایان بازی"""
        if self._time_update_timer:
            self._time_update_timer = None
            
        if self._time_text:
            self._time_text.delete()
            self._time_text = None
    ...
    def on_transition_out(self) -> None:
        """پاک کردن منابع هنگام خروج از بازی"""
        if self._time_update_timer:
            self._time_update_timer = None
