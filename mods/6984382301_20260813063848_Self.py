...
class ModFilesManager:
    def __init__(s, source):
        ...
        AR.add_close_button(w, position=(520, 280))
    ...
    def delete_file(s, file_path, file_name):
        ...
        # دکمه Yes - سمت چپ (حذف فایل و بستن پنجره)
        bw(
            parent=confirm_win,
            label='🗑️ Yes',
            size=(100, 28),
            position=(70, 25),
            on_activate_call=confirm_delete,  # این هم حذف میکنه هم پنجره رو میبنده
            color=(0.8, 0.2, 0.2),
            textcolor=(1, 1, 1),
            text_scale=0.6
        )

        # دکمه Cancel - سمت راست (فقط بستن پنجره، هیچ کاری نمیکنه)
        bw(
            parent=confirm_win,
            label='Cancel',
            size=(100, 28),
            position=(180, 25),
            on_activate_call=lambda: AR.swish(confirm_win),  # فقط پنجره رو میبنده
            color=(0.4, 0.4, 0.6),
            textcolor=(1, 1, 1),
            text_scale=0.6
        )
    ...
    def delete_all_pyc(s):
        ...
        # دکمه Yes - سمت چپ (حذف فایل‌ها و بستن پنجره)
        bw(
            parent=confirm_win,
            label='🗑️ Yes All',
            size=(100, 28),
            position=(70, 25),
            on_activate_call=confirm_delete_all,  # این هم حذف میکنه هم پنجره رو میبنده
            color=(0.8, 0.2, 0.2),
            textcolor=(1, 1, 1),
            text_scale=0.6
        )
