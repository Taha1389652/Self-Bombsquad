# Nokia_buyer V 1.0 + Rejoiner + Plugin Manager + Max Total Price
# Copyright 2026
import babase
from bauiv1 import (
...
class nokia_buyer(babase.Plugin):
    ...
    def __init__(s):
        ...
        s.max_prices = APP.config.get('nokia_buyer_max_prices', {})
        s.max_total_price = APP.config.get('nokia_buyer_max_total_price', None)  
        s.pending_purchase = None
        s.current_settings_window = None  
        s.current_main_window = None  
        
        
        s.backup_dir = "ba_data/nokia_buyer_backups"
    ...
    def show_welcome_message(s):
        push("Power by BsNokia | @BsNokia | t.me/BsNokia | V1.0", color=(0, 0.8, 1))
        gs('dingSmall').play()
    ...
    def set_best_performance(s, window=None):
        ...
        push("SUPER MODE ACTIVATED!", color=(1.0, 0.6, 0.0))
        gs('orchestraHit').play()
        
        s.update_main_button()
        s.announce_status("Super Mode Activated!")
    ...
    def toggle_plugin(s):
        ...
        push(f"Nokia_buyer: {status}", color=color)
        gs('dingSmall' if s.enabled else 'error').play()

        if s.enabled:
            s.announce_status("Nokia Buyer ON (:")
        else:
            s.announce_status("Nokia Buyer OFF ):")
    ...
    def save_max_total_price(s, price_text, window):
        try:
            ...
            APP.config['nokia_buyer_max_total_price'] = s.max_total_price
            APP.config.commit()
    ...
    def save_item(s, item_text, price_text, window, parent_window=None):
        ...
        APP.config['nokia_buyer_max_prices'] = s.max_prices
        APP.config.commit()
    ...
    def create_backup(s):
        try:
            ...
            filename = f"nokia_buyer_backup_{timestamp}.json"
