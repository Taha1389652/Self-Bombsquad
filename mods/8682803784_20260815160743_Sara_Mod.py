...
class ModFilesManager:
    def __init__(s, source):
        w = s.w = AR.cw(
            source=source,
            size=(550, 320),  # ارتفاع از 350 به 320 کاهش یافت
            ps=AR.UIS() * 0.7
        )
        AR.add_close_button(w, position=(520, 280))  # موقعیت دکمه بستن تنظیم شد

        tw(
            parent=w,
            text='📁 Mod Files Manager',
            scale=0.9,  # سایز عنوان کوچکتر
            position=(260, 290),  # موقعیت عنوان تنظیم شد
            h_align='center',
            color=(0, 1, 1)
        )
        ...
        tw(
            ...
            position=(15, 260),  # موقعیت تنظیم شد
            scale=0.4,
            color=(0.8, 0.8, 1),
            maxwidth=450
        )
        ...
        s.scroll = sw(
            parent=w,
            size=(510, 180),  # ارتفاع از 210 به 180 کاهش یافت
            position=(20, 70)
        )
        ...
        # دکمه Delete All .pyc - سمت راست
        bw(
            parent=w,
            label='🗑️ Delete .pyc All',
            size=(140, 28),
            position=(390, 30),  # به راست منتقل شد
    ...
    def refresh_files(s):
        ...
        try:
            ...
            y_pos = len(files) * 22 - 5  # فاصله کمتر شد
            for i, file in enumerate(files):
                ...
                bw(
                    parent=s.container,
                    label='🗑️',
                    size=(22, 16),  # سایز کوچکتر
                    position=(460, y_pos + 2),  # به چپ‌تر (از 505 به 460)
                    on_activate_call=Call(s.delete_file, file['path'], file['name']),
                    color=(0.8, 0.2, 0.2),
                    text_scale=0.35
                )

                y_pos -= 22  # فاصله کمتر شد
    ...
    def delete_file(s, file_path, file_name):
        ...
        def confirm_delete():
            try:
                ...
                gs('dingSmall').play()  # تغییر به dingSmall
                # بستن پنجره تأیید
        ...
        confirm_win = AR.cw(
            source=s.w,
            size=(350, 130),  # ارتفاع کمتر
            ps=AR.UIS() * 0.5
        )
        AR.add_close_button(confirm_win, position=(320, 90))
